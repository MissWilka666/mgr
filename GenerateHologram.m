function [img_input,img_target,holo] = GenerateHologram(img,Z,lambda,dx,type,M,tt)
...
if strcmp(type,'phs')
    ...
    u_out = AS_propagate_p(holo,Z,RI,lambda,dx);  %%%%% tutaj zamiast holo bylo abs(u_in).^2
    % remove padding
    u_out = u_out(padS+1:padS+imS,padS+1:padS+imS);
    img_input = angle(u_out)+pi;
else
    % backpropagate only the amplitude (sqrt of hologram)
    u_out = AS_propagate_p(sqrt(holo),Z,RI,lambda,dx); %%%%%% tutaj zamiast sqrt(holo) bylo abs(u_in)
    % remove padding
    u_out = u_out(padS+1:padS+imS,padS+1:padS+imS);
    img_input = abs(u_out);
end
holo=holo(padS+1:padS+imS,padS+1:padS+imS);  %%%% To bylo za wczesnie
end